...
ammo_pickups = []
mannequins = []
furniture = []  # Replaces obstacles for the house theme
walls = []  # Internal room partition walls
mannequins_killed = 0
...
MAP_SIZE = 1200 # Smaller map for a house feel
...
def init_game():
    ...
    # Init House Furniture (Abandoned theme)
    furniture = []
    # Add some Sofas and Almirahs in rooms (Manual placement for better layout)
    ...
    # Random scattered items (avoiding 0,0)
    # Bed and Sofa pairs against walls
    # East Side
    ...
    # Add Staircase (in room without bed - Top Left)
    furniture.append({'type': 'staircase', 'pos': [-1150, 800, 0], 'rot': 90, 'size': [300, 200, 200]})
    
    # Add Dining Set (Southeast Room - Room 4)
    # Dining Table
    table_pos = [800, -700, 0]
    ...
    # Init Internal Walls (Cross pattern creating 4 rooms with 2 doors per wall)
    walls = []
    door_width = 200  # Wider doors for easier passage
    wall_thickness = 20
    door_spacing = 400  # Distance from center to each door
    
    # Vertical wall (North-South) with 2 doors
...
def update_game_logic():
    ...
    # Spawn Mannequins - Initial batch handled in init. 
    # Replenishments handled in bullet collision logic below.
        ...
        for m in mannequins:
            ...
            if not hit and mdist < 40:
                ...
                if mannequins_killed >= MANNEQUIN_COUNT:
                    door_visible = True
                    # Door in corner of NE room (side of room, moved left)
...
def draw_door():
    ...
    # Facing into the room (assuming North Wall -> facing South)
    glRotatef(180, 0, 0, 1) # Rotate to face inward (if needed, or just flat on wall)
    # Actually, North Wall is +Y. So door normal should point -Y.
    # Default cube is centered. 
    ...
    # Simpler Frame: Just a backplate slightly larger?
    
    # Let's draw Frame as 3 parts
    # Top
...
def draw_sofa(x, y, z, rot, sx, sy, sz):
    ...
    # Base
    glColor3f(0.3, 0.1, 0.1) # Dark leather
...
def draw_almirah(x, y, z, rot, sx, sy, sz):
    ...
    glutSolidCube(30)
    # Door line - using a thin cube since GL_LINES might be avoided or invisible if too thin, 
    # but actual GL_LINES with default width is usually fine. Let's use a thin black strip (cube).
...
def draw_staircase(x, y, z, rot, width, height, steps):
    ...
    for i in range(steps):
        ...
        # 2. Tread (Top surface - Lighter to catch light)
        ...
        # 3. Deep Shadow (At the intersection of step and riser above)
...
def draw_dining_chandelier(x, y, z):
    ...
    # Light Beam (Semi-transparent yellow cone)
    glEnable(GL_BLEND)
    ...
    glPopMatrix()
    # Remove glDisable(GL_BLEND) if the rest of UI needs it, 
    # but better to let showScreen manage it. 
    # For now, let's keep it consistent with other transparent calls.
...
def draw_chandelier():
    ...
    glPushMatrix()
    glTranslatef(0, 0, 250)  # Center of house, high up
    
    # Central chain/rod (Dark metal) - extends upward
    glColor3f(0.2, 0.2, 0.2)
    glPushMatrix()
    glTranslatef(0, 0, 0)  # Start at chandelier center
    gluCylinder(gluNewQuadric(), 4, 4, 80, 10, 1)  # Extends upward in +Z
    glPopMatrix()
    
    # Main body (ornate sphere) - larger and more detailed
    glColor3f(0.3, 0.25, 0.15)  # Bronze-ish
...
def draw_player():
    """Draw a humanoid player character at player position"""
    glPushMatrix()
    glTranslatef(char_pos[0], char_pos[1], 0)
    glRotatef(char_rotation, 0, 0, 1)
    
    # Legs
    glColor3f(0.2, 0.2, 0.3)  # Dark blue pants
    # Left leg
    glPushMatrix()
    glTranslatef(-8, 0, 15)
    ...
    # Torso
    glColor3f(0.4, 0.3, 0.25)  # Brown shirt
    glPushMatrix()
    glTranslatef(0, 0, 45)
...
def draw_house():
    # Floor - Dark wood with Black Lines
    plank_width = 80
    gap = 5
    ...
    # Windows on external walls - with grids
    # Window positions
...
def draw_character():
    ...
    # Legs (Dark blue pants) - Side-by-side along Y axis
    ...
    # Torso (Brown shirt) - Wide along Y axis
    ...
    # Arms (Skin tone) - Shoulders along Y axis
    ...
    glutSolidCube(30)
    glPopMatrix()
    # Right arm (Holding flashlight position)
    glPushMatrix()
    glTranslatef(0, 15, 50)
    glScalef(5/30, 5/30, 25/30)
    glutSolidCube(30)
    ...
    # Hair (Black) - Shifted back slightly from front (+X)
    glColor3f(0.05, 0.05, 0.05)
    glPushMatrix()
    glTranslatef(-2, 0, 78)
    glScalef(1.0, 1.0, 0.8)
    ...
    # Flashlight (projects forward along +X)
    if flashlight_on:
        glEnable(GL_BLEND)
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
        glPushMatrix()
        glTranslatef(12, 15, 50) # In front of right arm
        glRotatef(90, 0, 1, 0)   # Points to +X (front)
